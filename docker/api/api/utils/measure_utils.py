...
from main import db
from utils.flexstring import validateConfig, flexstringParse
import json
...
def createDefaultGroup(display_id, configuration, replicats_id=None, deduplication=True):
# Create a new default_group (includes Deduplication)
    ...
    if deduplication:
        m = displayGroupHasDisplay.query.filter(displayGroupHasDisplay.display_id == display_id, displayGroupHasDisplay.configuration == json.dumps(configuration), displayGroupHasDisplay.displayGroup.has(displayGroup.is_default == 1)).first()
    ...
    if (not deduplication) or (not m):
        ...
        ok, err = validateConfig(configuration,d.varlist)
        if not ok:
            return dR_(0,etxt="Invalid configuration for display '"+str(d.id)+"': "+err)
        ...
        db.session.flush()
            # Create Group
        m = displayGroupHasDisplay(g.id,display_id,configuration)
...
def getMeasures(district_id=None, region_id=None):
    ...
    for dhg in dhgs:
        ...
        ok, res, epos = flexstringParse(dhg.displayGroup.displayLink[0].display.flexstring_german, dhg.displayGroup.displayLink[0].configuration)
        if not ok:
            return jsonify({"status": "InternalServerError", "error": "flexstringParseError for string '"+dhg.displayGroup.displayLink[0].display.flexstring_german+"' with configuration '"+str(dhg.displayGroup.displayLink[0].configuration)+"': "+res+" at Position "+str(epos)}), 500     
        ...
        res = {
            ...
            "display": {
                ...
                "title": dhg.displayGroup.displayLink[0].display.category.name,
                "subtitle":dhg.displayGroup.displayLink[0].display.subtitle_german,
                "varlist": dhg.displayGroup.displayLink[0].display.varlist,
                "text": res,
